import Link from "next/link";
import type { Article } from "@prisma/client";
import { getCategoryLabel } from "@/lib/categories";

export function ArticleCard({ article, size = "md" }: { article: Article; size?: "lg" | "md" | "sm" }) {
  const isLg = size === "lg";

  return (
    <Link
      href={`/articulos/${article.slug}`}
      className="group flex h-full flex-col overflow-hidden rounded-2xl border border-gold/15 bg-noir-surface/40 transition duration-300 hover:-translate-y-1 hover:border-gold/50 hover:bg-noir-surface hover:shadow-xl hover:shadow-black/30"
    >
      {article.coverImage && (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={article.coverImage}
          alt=""
          className={`w-full shrink-0 object-cover ${isLg ? "h-56" : "h-36"}`}
        />
      )}
      <div className="flex flex-1 flex-col p-6">
        <div className="mb-3 flex items-center gap-3 font-ui text-xs uppercase tracking-wide text-gold-dim">
          <span className="rounded-full border border-gold/30 px-2.5 py-1 text-gold">
            {getCategoryLabel(article.category)}
          </span>
          {article.sign && <span>{article.sign}</span>}
          <span>· {article.readingTimeMin} min</span>
        </div>
        <h3 className={`font-display leading-snug text-gold-pale group-hover:text-gold ${isLg ? "text-2xl sm:text-3xl" : "text-xl"}`}>
          {article.title}
        </h3>
        <p
          className={`mt-3 font-body text-gold-pale/80 [-webkit-box-orient:vertical] [-webkit-line-clamp:3] [display:-webkit-box] overflow-hidden ${isLg ? "text-lg" : "text-base"}`}
        >
          {article.excerpt}
        </p>
        <p className="mt-auto pt-4 font-ui text-xs text-gold-dim">Por Zodiac Noir</p>
      </div>
    </Link>
  );
}
